...
voting_machines[voting_machine_nums] = voting_machines[voting_machine_nums].astype(
    'int')
# Convert ID to int
...
def izgbs(vote_loc, total_num, start, min_val, alpha):
    ...
    while (hyps_rem):
        ...
        rep_nums = final_loc_results.groupby(['Replication'])['Waiting_Time'].agg([
            'max', 'mean']).reset_index()
        # Rename fields
        rep_nums.rename(
            columns={'max': 'Max_Waiting_Time', 'mean': 'Avg_Waiting_Time'}, inplace=True)
        ...
        batch_avgs = rep_nums.groupby(
            'Batch')[['Max_Waiting_Time', 'Avg_Waiting_Time']].agg('mean').reset_index()
        ...
        feasible_df.loc[feasible_df.Machines ==
                        num_test, 'BatchAvgMax'] = max_avg
...
for i in range(1, num_locations):
    ...
    if not loc_feas.empty:
        ...
        loc_df_results.loc[loc_df_results.Locations ==
                           str(i), 'Resource'] = mach_min
        loc_df_results.loc[loc_df_results.Locations == str(
            i), 'Exp. Avg. Wait Time'] = loc_feas_min.iloc[0]['BatchAvg']
        loc_df_results.loc[loc_df_results.Locations == str(
            i), 'Exp. Max. Wait Time'] = loc_feas_min.iloc[0]['BatchAvgMax']
        # loc_df_results.head()
        print(loc_df_results)
...
# number of out of compliance voters at each location, not currently being used
